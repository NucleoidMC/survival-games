...
public class DebugJigsawMapper {
	public static void map(SurvivalGamesConfig config, Long2ObjectMap<List<PoolElementStructurePiece>> piecesByChunk, ChunkBox townArea, ChunkMask totalArea) {
		...
		for (int chunkX = -chunkRad; chunkX < chunkRad; chunkX++) {
		    for (int chunkZ = -chunkRad; chunkZ < chunkRad; chunkZ++) {
				...
				List<PoolElementStructurePiece> pieces = piecesByChunk.get(new ChunkPos(chunkX, chunkZ).pack());
		        ...
		        for (int x = chunkStartX; x < chunkStartX + 16; x++) {
		            for (int z = chunkStartZ; z < chunkStartZ + 16; z++) {
						...
						for (PoolElementStructurePiece piece : pieces) {
							...
							if (box.intersects(x - 2, z - 2, x + 2, z + 2)) {
								color = 0xcccccc;

								String location = ((SinglePoolElementAccessor)piece.getElement()).getLocation().left().get().toString();
